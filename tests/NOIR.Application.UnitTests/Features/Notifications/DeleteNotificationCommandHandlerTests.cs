...
/// <summary>
/// Unit tests for DeleteNotificationCommandHandler.
/// </summary>
public class DeleteNotificationCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_NotificationBelongsToOtherUser_ShouldReturnNotFound()
    {
        // Arrange
        SetupAuthenticatedUser("different-user");
        var command = new DeleteNotificationCommand(Guid.NewGuid());

        // NotificationByIdSpec filters by userId, so it returns null for other user's notifications
        _repositoryMock
            .Setup(x => x.FirstOrDefaultAsync(It.IsAny<ISpecification<Notification>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((Notification?)null);
    ...
    [Fact]
    public async Task Handle_ValidNotification_ShouldDeleteSuccessfully()
    {
        // Arrange
        SetupAuthenticatedUser();
    ...
    [Fact]
    public async Task Handle_UnreadNotification_ShouldUpdateUnreadCount()
    {
        // Arrange
        SetupAuthenticatedUser();
    ...
    [Fact]
    public async Task Handle_ReadNotification_ShouldNotUpdateUnreadCount()
    {
        // Arrange
        SetupAuthenticatedUser();
        var notification = CreateTestNotification(isRead: true);
