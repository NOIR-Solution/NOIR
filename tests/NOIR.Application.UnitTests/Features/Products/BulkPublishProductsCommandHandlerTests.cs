...
public class BulkPublishProductsCommandHandlerTests
{
    ...
    public async Task Handle_WithMixedStatusProducts_ShouldReturnPartialSuccess()
    {
        ...
        result.Value.Errors.Should().Contain(e => e.ProductId == activeId);
    ...
    public async Task Handle_WithAllNonExistentProducts_ShouldReturnAllErrors()
    {
        ...
        // Assert
        result.IsSuccess.Should().BeTrue(); // Operation itself succeeds, just no products to publish
    ...
    public async Task Handle_WithAllActiveProducts_ShouldReturnAllErrors()
    {
        ...
        result.Value.Failed.Should().Be(2);
        result.Value.Errors.Should().HaveCount(2);
        result.Value.Errors.Should().AllSatisfy(e => e.Message.Should().Contain("not in Draft status"));
    }
